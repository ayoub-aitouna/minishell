...
void	split_by_op(char *ptr, t_tree **tree)
{
	...
	*tree = new_tree_node(op, ptr);
	if (op)
	{
		split_by_op(remove_outer_brackets(copy_str(0, i - 1, ptr)),
					&(*tree)->left);
...
int	main(int ac, char **av)
{
	...
	{
		t_tree *tree = NULL;
		printf("removed brackets %s \n",
				remove_outer_brackets(ft_strdup(av[1])));
		split_by_op(remove_outer_brackets(ft_strdup(av[1])), &tree);
		tree_iterat(tree, 1);
	}
}