...
int	recursive_exec(t_tree *tree, int in_fd, int **procces_id, int *len,
		int ends_count, int *index, int w)
{
	int	pid;
	int	status;
	int	fd[2];
	...
	{
		status = recursive_exec(tree->left, in_fd, procces_id, len, ends_count,
				index, 1);
		status = wait_all_proccess(*procces_id, *len);
		if (tree->op == 2 && status != 0)
			status = recursive_exec(tree->right, in_fd, procces_id, len,
					ends_count, index, 1);
...
void	shell_exec(t_tree *tree)
{
	...
	int status = recursive_exec(tree, STDIN_FILENO, &proccess_id, &len,
			ends_count, &index, 0);
